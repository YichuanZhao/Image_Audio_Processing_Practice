...
int main(int argc, char* argv[]) {
	...
	VideoCapture input_cap("input4_2.avi");

    Mat zero = imread("digits/0.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_0, bw_0, dst_0;
    cvtColor(zero,gray_0,CV_RGB2GRAY);
    threshold(gray_0, bw_0, 200, 255.0, THRESH_BINARY);

    Mat one = imread("digits/1.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_1, bw_1, dst_1;
    cvtColor(one,gray_1,CV_RGB2GRAY);
    threshold(gray_1, bw_1, 200, 255.0, THRESH_BINARY);

    Mat two = imread("digits/2.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_2, bw_2, dst_2;
    cvtColor(two,gray_2,CV_RGB2GRAY);
    threshold(gray_2, bw_2, 200, 255.0, THRESH_BINARY);

    Mat three = imread("digits/3.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_3, bw_3, dst_3;
    cvtColor(three,gray_3,CV_RGB2GRAY);
    threshold(gray_3, bw_3, 200, 255.0, THRESH_BINARY);

    Mat four = imread("digits/4.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_4, bw_4, dst_4;
    cvtColor(four,gray_4,CV_RGB2GRAY);
    threshold(gray_4, bw_4, 200, 255.0, THRESH_BINARY);

    Mat five = imread("digits/5.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_5, bw_5, dst_5;
    cvtColor(five,gray_5,CV_RGB2GRAY);
    threshold(gray_5, bw_5, 200, 255.0, THRESH_BINARY);

    Mat six = imread("digits/6.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_6, bw_6, dst_6;
    cvtColor(six,gray_6,CV_RGB2GRAY);
    threshold(gray_6, bw_6, 200, 255.0, THRESH_BINARY);    

    Mat seven = imread("digits/7.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_7, bw_7, dst_7;
    cvtColor(seven,gray_7,CV_RGB2GRAY);
    threshold(gray_7, bw_7, 200, 255.0, THRESH_BINARY);

    Mat eight = imread("digits/8.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_8, bw_8, dst_8;
    cvtColor(eight,gray_8,CV_RGB2GRAY);
    threshold(gray_8, bw_8, 200, 255.0, THRESH_BINARY);

    Mat nine = imread("digits/9.jpeg", CV_LOAD_IMAGE_COLOR);

    Mat gray_9, bw_9, dst_9;
    cvtColor(nine,gray_9,CV_RGB2GRAY);
    threshold(gray_9, bw_9, 200, 255.0, THRESH_BINARY);
	...
	VideoWriter output_cap("output4_2.avi", 
    ...
    Mat frame_gray, frame_bw;

    Mat upper_crop;

    vector<Point> coordinates_0, coordinates_1, coordinates_2, coordinates_3, coordinates_4, 
                  coordinates_5, coordinates_6, coordinates_7, coordinates_8, coordinates_9;
	while(1){
        input_cap >> frame;
        if(init == 0){
        ...
        threshold(frame_gray, frame_bw, 150, 255.0, THRESH_BINARY);
        //erode(frame_bw, dst_0, bw_0);
        
        Rect roi(10,7,60,20);
        frame_bw(roi).copyTo(upper_crop); 

        erode(upper_crop, dst_0, bw_0);
        findNonZero(dst_0, coordinates_0);
        //cout << coordinates_0 <<endl;
        for(int i=0; i<coordinates_0.size(); i++){
        	putText(frame,"0",Point(coordinates_0[i].x+150, coordinates_0[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }

        erode(upper_crop, dst_1, bw_1);
        findNonZero(dst_1, coordinates_1);
        //cout << coordinates_1 <<endl;
        for(int i=0; i<coordinates_1.size(); i++){
        	putText(frame,"1",Point(coordinates_1[i].x+150, coordinates_1[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

        erode(upper_crop, dst_2, bw_2);
        findNonZero(dst_2, coordinates_2);
        //cout << coordinates_2 <<endl;
        for(int i=0; i<coordinates_2.size(); i++){
        	putText(frame,"2",Point(coordinates_2[i].x+150, coordinates_2[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

        erode(upper_crop, dst_3, bw_3);
        findNonZero(dst_3, coordinates_3);
        //cout << coordinates_3 <<endl;
        for(int i=0; i<coordinates_3.size(); i++){
        	putText(frame,"3",Point(coordinates_3[i].x+150, coordinates_3[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

        erode(upper_crop, dst_4, bw_4);
        findNonZero(dst_4, coordinates_4);
        //cout << coordinates_4 <<endl;
        for(int i=0; i<coordinates_4.size(); i++){
        	putText(frame,"4",Point(coordinates_4[i].x+150, coordinates_4[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

		erode(upper_crop, dst_5, bw_5);
		findNonZero(dst_5, coordinates_5);
		//cout << coordinates_5 <<endl;
        for(int i=0; i<coordinates_5.size(); i++){
        	putText(frame,"5",Point(coordinates_5[i].x+150, coordinates_5[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

		erode(upper_crop, dst_6, bw_6);
		findNonZero(dst_6, coordinates_6);
		//cout << coordinates_6 <<endl;
        for(int i=0; i<coordinates_6.size(); i++){
        	putText(frame,"6",Point(coordinates_6[i].x+150, coordinates_6[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

		erode(upper_crop, dst_7, bw_7);
		findNonZero(dst_7, coordinates_7);
		//cout << coordinates_7 <<endl;
        for(int i=0; i<coordinates_7.size(); i++){
        	putText(frame,"7",Point(coordinates_7[i].x+150, coordinates_7[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

		erode(upper_crop, dst_8, bw_8);
		findNonZero(dst_8, coordinates_8);
		//cout << coordinates_8 <<endl;
        for(int i=0; i<coordinates_8.size(); i++){
        	putText(frame,"8",Point(coordinates_8[i].x+150, coordinates_8[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
        

		erode(upper_crop, dst_9, bw_9);
		findNonZero(dst_9, coordinates_9);
		//cout << coordinates_9 <<endl;
        for(int i=0; i<coordinates_9.size(); i++){
        	putText(frame,"9",Point(coordinates_9[i].x+150, coordinates_9[i].y + 10),1,0.8,Scalar(255,255,255),1,8);
        }
